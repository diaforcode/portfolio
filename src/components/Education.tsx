...
import { education } from "@/app/data/constants";
    ...
    <section id="education" className="relative w-full py-24 px-4 md:px-8">
      ...
      >
        {education.map((item, index) => (
          ...
          >
            <Card className="group border border-white/10 bg-white/5 backdrop-blur-md shadow-xl hover:shadow-2xl transition-all duration-300 rounded-2xl text-white overflow-hidden">
              <CardContent className="flex flex-col gap-4 p-6 text-left">
                <h3 className="text-xl font-semibold text-white">
                  {item.degree}
                </h3>
                <p className="text-sm text-white/80">{item.school}</p>
                <p className="text-sm italic text-white/60">{item.date}</p>
                <p className="text-sm mt-2 leading-relaxed text-white/80 whitespace-pre-wrap transition-all duration-300 line-clamp-2 group-hover:line-clamp-none">
                  {item.desc}
                </p>
              </CardContent>
            </Card>
